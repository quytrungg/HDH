...
{ 
	...
	for (int i = 0; i < 10; i++)
		opfile[i] = NULL;
	opfile[index++] = this->Open("input", 2);
	opfile[index++] = this->Open("output", 3);
	this->Create("input", 0);
...
OpenFile* FileSystem::Open(char *name, int type){
	int slot = this->findEmptyPosition();
	Directory *directory = new Directory(NumDirEntries);
	//OpenFile *openFile = NULL;
	int sector;
	...
	directory->FetchFrom(directoryFile);
	sector = directory->Find(name);
	if (sector >= 0) opfile[slot] = new OpenFile(sector, type);	// name was found in directory 
	delete directory;
	//index++;
	return opfile[slot];				// return NULL if not found
}
